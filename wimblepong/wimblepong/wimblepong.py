...
class Ball():
    ...
    def reset_ball(self):
        ...
        #self.x = self.GAME_AREA_RESOLUTION[0]//2 + int((2.0*np.random.random()-1) * (self.GAME_AREA_RESOLUTION[0]/2.0 - 20))
        #self.y = int(np.random.random() * (self.GAME_AREA_RESOLUTION[1]-self.h*4)) + 2*self.h + self.SCOREBOARD_HEIGHT
        self.last_touch = 0
        self.update_rect()
...
class Player():
    ...
    def __init__(self, player_number, GAME_AREA_RESOLUTION, SCOREBOARD_HEIGHT):
        ...
        self.touch = False      # check if player touch the ball
    ...
    def reset(self):
        ...
        self.y = self.GAME_AREA_RESOLUTION[1]//2 + self.SCOREBOARD_HEIGHT
        #self.y = int(
        #    np.random.random() * (self.GAME_AREA_RESOLUTION[1] - self.h * 2)) + 1 * self.h + self.SCOREBOARD_HEIGHT
        self.update_rect()
        self.touch = False
...
class Wimblepong(gym.core.Env):
    ...
    def _step_collisions(self):
        ...
        if p1_collide and self.player1.touch is False:
            self.player1.touch = True
        if p2_collide and self.player2.touch is False:
            self.player2.touch = True
    ...
    def _step_get_state(self, player1_reward, player2_reward):
        ...
        if self.opponent:
            ...
            # return np asarray
            return np.asarray([observation_left, observation_right]), np.asarray([player1_reward, player2_reward])
    ...
    def reset(self):
        ...
        if self.opponent is None:
            ...
            # return np as arrray
            return np.asarray([self._get_observation(1), self._get_observation(2)])
        else:
