...
parser.add_argument('--exp_name', type=str, default='compare_diff_agents')
...
# player list
# path_list = ['../data/fpo_clip_entropy(0)_pong_2p/fpo_clip_entropy(0)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.001)_pong_2p/fpo_clip_entropy(0.001)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.003)_pong_2p/fpo_clip_entropy(0.003)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.01)_pong_2p/fpo_clip_entropy(0.01)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.03)_pong_2p/fpo_clip_entropy(0.03)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.1)_pong_2p/fpo_clip_entropy(0.1)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.3)_pong_2p/fpo_clip_entropy(0.3)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_replay(1)_pong_2p/fpo_clip_replay(1)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_replay(10)_pong_2p/fpo_clip_replay(10)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_replay(30)_pong_2p/fpo_clip_replay(30)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_replay(100)_pong_2p/fpo_clip_replay(100)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.03)_replay(1)_pong_2p/fpo_clip_entropy(0.03)_replay(1)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.03)_replay(10)_pong_2p/fpo_clip_entropy(0.03)_replay(10)_pong_2p_s{}_backup/',
#                '../data/fpo_clip_entropy(0.03)_replay(100)_pong_2p/fpo_clip_entropy(0.03)_replay(100)_pong_2p_s{}_backup/',]
...
path_list = ['../data/fpo_clip_entropy(0.1)_pong_2p/fpo_clip_entropy(0.1)_pong_2p_s{}_backup/',
...
for i in range(len(path_list)):
    if path_list[i] is 'simpleai':
        get_actions.append([])
        for s in range(3):
            simple_ai = SimpleAi(env, 1)
            get_actions[-1].append(simple_ai.get_action)
        players.append(path_list[i])
    else:
        get_actions.append([])
        for s in range(3):
            loc = path_list[i].format((s + 1) * 100)
            graph = tf.Graph()
            with graph.as_default():
                _, op = load_policy_and_env(loc, itr='last')
                get_actions[-1].append(op)
        players.append(path_list[i])
        if 'nfsp' in path_list[i]:
            get_actions.append([])
            for s in range(3):
                loc = path_list[i].format((s + 1) * 100)
                graph = tf.Graph()
                with graph.as_default():
                    ...
                    get_actions[-1].append(op)
            players.append(path_list[i]+'fic')

for i in range(len(players)):
    for s in range(3):
        a1 = get_actions[i][s](np.array([0,0,0,0,0,0]))

# record matching results
match_payoff = np.zeros((len(players), len(players), 3, 3))
        ...
        for s1 in range(3):
            for s2 in range(3):
                [Awin_rate1, Bwin_rate1, draw_rate1] = calculate_play_result(
                    env, get_actions[i][s1], get_actions[j][s2], rounds=int(local_rounds / 2))
                [Awin_rate2, Bwin_rate2, draw_rate2] = calculate_play_result(
                    env, get_actions[j][s2], get_actions[i][s1], rounds=int(local_rounds / 2))
                ...
                logger.log_tabular('Match', players[i].format((s1+1)*100) +
                                   '_vs_' + players[j].format((s2+1)*100))
...
match_payoff = match_payoff - np.transpose(match_payoff, (1, 0, 2, 3))

if proc_id() == 0:
    print(match_payoff)
    np.save('../data/' + args.exp_name + '/match_payoff', match_payoff)
    np.save('../data/' + args.exp_name + '/players', players)
