...
def mlp_categorical_q(x, hidden_sizes, activation, output_activation, action_space):
...
def mlp_fic_actor(x, a, hidden_sizes=(64,64), activation=tf.tanh,
                  output_activation=None, action_space=None):
    with tf.variable_scope('fic'):
...
Actor-Critics
"""
def mlp_q_v(x, hidden_sizes=(64,64), activation=tf.nn.relu,
                     output_activation=None, action_space=None):
    ...
    with tf.variable_scope('q'):
        q = mlp_categorical_q(x, hidden_sizes, activation, output_activation, action_space)
    with tf.variable_scope('adv'):
        adv = q-tf.expand_dims(v, -1)
